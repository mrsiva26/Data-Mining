  ...
  for(j in 1:nrow(income_mini) )
  {
    ...
    if(i!=j)
    {
      ...
      dist = 
        (row1$age-row2$age)^2 + 
        workclassdiff^2 + 
        (row1$fnlwgt-row2$fnlwgt)^2 + 
        (row1$education_cat-row2$education_cat)^2 + 
        occupationdiff^2 +
        relationshipdiff^2 +
        racediff^2 +
        genderdiff^2 +
        (row1$capital_gain-row2$capital_gain)^2 +
        (row1$capital_loss-row2$capital_loss)^2 +
        (row1$hour_per_week-row2$hour_per_week)^2 +
        native_countrydiff^2
...
print(result_income)
write.csv(result_income, file = "C:/Users/Siva/Desktop/Data Mining/Homework 1/Income_euclid_output.csv", row.names = TRUE)